...
const ADMIN_PHONE = "9633825283"; // replace with your WhatsApp number
...
export default function PropertyDetailScreen() {
  ...
  if (!property) {
    return (
      <View className="flex-1 items-center justify-center bg-white dark:bg-neutral-950">
        <Text className="text-gray-500 dark:text-neutral-400">Property not found</Text>
      </View>
    );
  }
          ...
          <SafeAreaView className="absolute top-0 left-0 right-0">
            <View className="flex-row items-center justify-between px-4 pt-2">
              <TouchableOpacity
                ...
                className="w-10 h-10 bg-white/95 dark:bg-neutral-900/95 rounded-full items-center justify-center border border-gray-200/80 dark:border-neutral-700"
                style={{ elevation: 3 }}
              >
                <Ionicons name="arrow-back" size={20} color={iconMuted} />
              </TouchableOpacity>
              <TouchableOpacity
                onPress={toggleSave}
              ...
              >
                <Ionicons
                  ...
                  size={20}
                  color={isSaved ? "#EF4444" : iconMuted}
                />
              </TouchableOpacity>
            </View>
          </SafeAreaView>
        ...
        >
          ...
          <View className="flex-row gap-2 mb-3 flex-wrap">
            ...
            {property.is_featured && (
              <View className="bg-amber-50 dark:bg-amber-950/40 px-3 py-1 rounded-full">
            ...
            {property.is_sold && (
              <View className="bg-red-50 dark:bg-red-950/40 px-3 py-1 rounded-full">
          ...
          {/* Specs Row */}
          <View className="flex-row justify-between bg-gray-50 dark:bg-neutral-900 rounded-2xl p-4 mb-5">
            <SpecItem
              icon="bed-outline"
              label="Beds"
          ...
          <Text className="text-gray-500 dark:text-neutral-400 text-sm leading-6 mb-1">
            {displayDesc}
          </Text>
          {isLongDesc && (
          ...
          <View className="flex-row items-center gap-2 mb-4">
            ...
            <Text className="text-gray-500 dark:text-neutral-400 text-sm flex-1">
          ...
          >
            <WebView
              ...
              pointerEvents="none"
            />
            <View className="absolute bottom-3 right-3 bg-white/90 dark:bg-neutral-900/90 px-3 py-1 rounded-full flex-row items-center gap-1">
              <Ionicons name="expand-outline" size={12} color="#9CA3AF" />
              <Text className="text-gray-600 dark:text-neutral-300 text-xs font-medium">
                Tap to expand
              </Text>
            </View>
          </TouchableOpacity>
          ...
          {isAdmin && (
            <View className="gap-3">
              <TouchableOpacity
                onPress={() =>
                  router.push(`/(root)/property/update/${property.id}`)
                }
                className="flex-row items-center justify-center gap-2 bg-white dark:bg-neutral-900 py-4 rounded-2xl border-2 border-blue-600"
              >
              ...
              <View className="flex-row gap-3">
                {!property.is_sold && (
                  <TouchableOpacity
                    ...
                    className="flex-1 flex-row items-center justify-center gap-2 bg-amber-50 dark:bg-amber-950/40 py-4 rounded-2xl border border-amber-200 dark:border-amber-800"
                  >
                    <Ionicons
                ...
                <TouchableOpacity
                  ...
                  className="flex-1 flex-row items-center justify-center gap-2 bg-red-50 dark:bg-red-950/40 py-4 rounded-2xl border border-red-100 dark:border-red-900/50"
                >
...
function SpecItem({
  ...
  return (
    <View className="items-center gap-1">
      ...
      <Text className="text-gray-900 dark:text-white font-bold text-sm">{value}</Text>
      <Text className="text-gray-400 dark:text-neutral-500 text-xs">{label}</Text>
    </View>
  );
}